...
const Inventory: React.FC<InventoryProps> = ({ data, setData, currentUser }) => {
  ...
  return (
    <div className="space-y-6 animate-fadeIn pb-10">
      <div className="flex flex-col md:flex-row justify-between items-stretch gap-4 bg-white p-6 rounded-[2.5rem] shadow-sm border border-slate-100">
        <div className="relative flex-1">
          <input type="text" placeholder="جستجوی نام یا کد کالا..." className="w-full pr-12 py-4 bg-slate-50 border-2 border-transparent rounded-2xl outline-none focus:border-indigo-500 font-bold transition-all text-sm shadow-inner" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
          <span className="absolute right-4 top-1/2 -translate-y-1/2 text-xl opacity-30">🔍</span>
        </div>
        <div className="flex gap-3">
          <button onClick={() => { setEditingProduct(null); setFormData({ code: '', name: '', buyPrice: '', shippingCost: '', marginPercent: '', quantity: '', date: getCurrentJalaliDate() }); setShowModal(true); }} className="flex-1 md:flex-none bg-indigo-600 text-white px-10 py-4 rounded-2xl font-black hover:bg-indigo-700 shadow-xl transition-all active:scale-95 text-center whitespace-nowrap">+ ثبت کالای جدید</button>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {filtered.map(p => {
          ...
          return (
            <div key={p.id} className="bg-white p-7 rounded-[3rem] border border-slate-100 shadow-sm hover:shadow-2xl transition-all relative overflow-hidden group">
              ...
              <div className="flex justify-between items-start mb-6">
                <div className="space-y-1">
                  <span className="text-[10px] font-black text-indigo-500 bg-indigo-50 px-2 py-0.5 rounded-md uppercase tracking-widest">{toPersianNumbers(p.code)}</span>
                  <h4 className="font-black text-slate-800 text-xl leading-tight group-hover:text-indigo-600 transition-colors">{p.name}</h4>
                  <p className="text-[10px] text-slate-400 font-bold">ثبت شده در: {toPersianNumbers(p.date)}</p>
                </div>
                {isLowStock && (
                  <div className="bg-red-100 text-red-600 p-2 rounded-xl text-lg animate-bounce" title="رو به اتمام">⚠️</div>
                )}
              </div>
              ...
              <div className="bg-slate-50 rounded-[2rem] p-5 mb-6 border border-slate-100">
                <div className="grid grid-cols-3 gap-2 text-center">
                  <div className="border-l border-slate-200">
                    <p className="text-[9px] font-black text-slate-400 mb-1">ورودی کل</p>
                    <p className="text-sm font-black text-slate-600">{toPersianNumbers(totalInventory)}</p>
                  </div>
                  <div className="border-l border-slate-200">
                    <p className="text-[9px] font-black text-indigo-400 mb-1">فروخته شده</p>
                    <p className="text-sm font-black text-indigo-700">{toPersianNumbers(sold)}</p>
                  </div>
                  <div>
                    <p className={`text-[9px] font-black mb-1 ${isLowStock ? 'text-red-500' : 'text-emerald-500'}`}>باقیمانده</p>
                    <p className={`text-lg font-black ${isLowStock ? 'text-red-600' : 'text-emerald-600'}`}>{toPersianNumbers(p.quantity)}</p>
                  </div>
                </div>
              ...
              <div className="flex justify-between items-center mb-6 px-2">
                <div><p className="text-[9px] font-black text-slate-400">قیمت واحد</p><p className="text-lg font-black text-indigo-950">{formatCurrency(p.sellPrice)}</p></div>
                <div className="text-left bg-emerald-50 px-3 py-1.5 rounded-xl border border-emerald-100">
                   <p className="text-[8px] font-black text-emerald-600 mb-0.5">درصد سود</p>
                   <p className="text-xs font-black text-emerald-700">{toPersianNumbers(p.marginPercent)}%</p>
                </div>
              </div>

              <div className="flex gap-2">
                <button onClick={() => { setEditingProduct(p); setFormData({ code: p.code, name: p.name, buyPrice: p.buyPrice.toString(), shippingCost: p.shippingCost.toString(), marginPercent: p.marginPercent.toString(), quantity: p.quantity.toString(), date: p.date }); setShowModal(true); }} className="flex-1 bg-slate-900 text-white py-3.5 rounded-2xl font-black text-xs hover:bg-black transition-all shadow-lg active:scale-95">📝 ویرایش کالا</button>
                <button onClick={() => { if(confirm('با حذف کالا تمامی سوابق مرتبط نیز از انبار حذف خواهد شد. مطمئن هستید؟')) setData({...data, products: data.products.filter(item => item.id !== p.id)}) }} className="bg-red-50 text-red-500 px-5 py-3.5 rounded-2xl font-black text-xs hover:bg-red-600 hover:text-white transition-all">🗑️</button>
              </div>
            </div>
          );
        })}
      ...
      {showModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-0 md:p-6 bg-slate-900/80 backdrop-blur-md">
          <div className="bg-white w-full h-full md:h-auto md:max-h-[95vh] md:max-w-2xl md:rounded-[3.5rem] shadow-2xl relative z-[110] flex flex-col animate-fadeIn overflow-hidden">
              ...
              <form id="product-form" onSubmit={saveProduct} className="space-y-8">
                ...
                <div className="p-6 md:p-8 bg-white rounded-[2.5rem] border-2 border-slate-100 shadow-inner space-y-6">
                  ...
                  <div className="bg-slate-900 rounded-[2rem] p-6 text-white flex flex-col md:flex-row justify-around items-center gap-4 border-4 border-indigo-600/20">
                    ...
                    <div className="text-center bg-white/5 p-4 rounded-2xl"><p className="text-[10px] font-black text-emerald-400 mb-1">قیمت نهایی فروش (برچسب):</p><p className="text-3xl font-black text-emerald-400">{formatCurrency(calculateFinalPrice())}</p></div>
                  </div>
                </div>
              </form>
            </div>
            ...
            <div className="px-6 py-6 md:px-10 md:py-8 bg-white border-t border-slate-100 flex gap-4">
              <button type="submit" form="product-form" className="flex-1 bg-indigo-600 text-white py-4 rounded-[1.5rem] font-black text-xl shadow-xl hover:bg-indigo-700 transition-all active:scale-95">✅ تایید و ثبت در انبار</button>
