    ...
    <div className="space-y-6 animate-slide-up pb-20">
        ...
        {filtered.map(inv => (
          <div key={inv.id} className="bg-white p-7 md:p-8 rounded-[2.5rem] shadow-sm border border-slate-100 hover:shadow-xl transition-all relative">
            ...
            <div className="flex gap-2.5">
              <button onClick={() => setShowPrintModal(inv)} className="flex-1 bg-indigo-50 text-indigo-600 py-3.5 rounded-2xl font-black text-xs hover:bg-indigo-600 hover:text-white">👁️ نمایش</button>
              <button onClick={() => handleEdit(inv)} className="flex-1 bg-blue-50 text-blue-600 py-3.5 rounded-2xl font-black text-xs hover:bg-blue-600 hover:text-white">📝 ویرایش</button>
              <button onClick={() => deleteInvoice(inv.id)} className="bg-red-50 text-red-500 px-4 rounded-2xl font-black text-sm">🗑️</button>
            </div>
          </div>
        ))}
      </div>
      ...
      {showPrintModal && (
        <div className="fixed inset-0 bg-slate-100/95 backdrop-blur-lg z-[3000] p-4 md:p-10 overflow-auto flex flex-col items-center">
          <div className="max-w-4xl w-full flex flex-wrap justify-center gap-3 no-print mb-8">
            <button onClick={() => window.print()} className="bg-slate-900 text-white px-8 py-4 rounded-2xl font-black text-sm">🖨️ چاپ فاکتور</button>
            <button onClick={() => setShowPrintModal(null)} className="bg-white text-slate-500 px-8 py-4 rounded-2xl font-black border-2 border-slate-200 text-sm">بازگشت</button>
          </div>
          <div ref={invoiceRef} className="invoice-container w-full max-w-[210mm] min-h-[297mm] p-12 md:p-16 rounded-xl bg-white shadow-2xl relative mb-20 border-t-[20px] border-slate-900">
             <div className="flex justify-between items-start mb-20">
                <div>
                   <h1 className="text-4xl md:text-6xl font-black text-slate-900 mb-2">سیرجان پوش</h1>
                   <p className="text-slate-400 font-bold tracking-widest text-xs">ثبت‌کننده: {showPrintModal.registeredBy || 'ناشناس'}</p>
                </div>
                <div className="bg-slate-50 p-8 rounded-[1.8rem] border-2 border-slate-100 min-w-[250px]">
                   <h2 className="text-2xl font-black text-indigo-600 mb-4 text-center">فـاکـتـور فـروش</h2>
             ...
             <div className="mb-12 p-8 bg-slate-900 rounded-[2.5rem] text-white">
                <p className="text-[10px] opacity-50 mb-2">مشتری گرامی:</p>
                <p className="text-2xl md:text-3xl font-black mb-1">{showPrintModal.customerName}</p>
                {showPrintModal.customerPhone && <p className="text-lg font-black text-emerald-400">{toPersianNumbers(showPrintModal.customerPhone)}</p>}
             </div>
             <table className="w-full border-collapse mb-12">
                <thead><tr className="bg-slate-100 text-slate-500 text-xs"><th className="p-5 text-right">شرح کالا</th><th className="p-5 text-center">تعداد</th><th className="p-5 text-center">واحد</th><th className="p-5 text-center">جمع</th></tr></thead>
                <tbody className="divide-y divide-slate-50 font-bold">
                   {showPrintModal.items.map((item, i) => (
                      <tr key={i}><td className="p-5">{item.name}</td><td className="p-5 text-center">{toPersianNumbers(item.quantity)}</td><td className="p-5 text-center">{toPersianNumbers(formatWithCommas(item.price))}</td><td className="p-5 text-center font-black text-indigo-600">{toPersianNumbers(formatWithCommas(item.price * item.quantity))}</td></tr>
                   ))}
                </tbody>
             </table>
             <div className="bg-slate-900 p-10 rounded-[3rem] text-white flex justify-between items-center shadow-2xl">
                <div>
                   <p className="text-[10px] opacity-50 mb-1">مبلغ نهایی قابل پرداخت:</p>
                   <p className="text-3xl md:text-5xl font-black text-emerald-400">{formatCurrency(showPrintModal.totalAmount)}</p>
                </div>
                <div className="opacity-30 italic font-black text-xl">تراکنش موفق</div>
             </div>
          </div>
        </div>
      )}
